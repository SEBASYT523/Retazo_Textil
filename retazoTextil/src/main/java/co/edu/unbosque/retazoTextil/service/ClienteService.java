...
public class ClienteService {
    ...
    public int create(ClienteDTO data) {
        ...
        if (data.getpedidosId() != null && !data.getpedidosId().isEmpty()) {
            List<Pedido> pedidos = data.getpedidosId().stream()
                    .map(pid -> pedidoRepo.findById(pid).orElse(null))
    ...
    public List<ClienteDTO> getAll() {
        return clienteRepo.findAll().stream().map(c -> {
            ...
            if (c.getPedidos() != null) {
                dto.setpedidosId(c.getPedidos().stream()
                        .map(Pedido::getId)
                        .collect(Collectors.toList()));
            }
            return dto;
    ...
    public int updateById(Integer id, ClienteDTO newData) {
        ...
        if (optCliente.isEmpty()) return 1;

        Cliente cliente = optCliente.get();

        if (newData.getPrimerNombre() != null) cliente.setPrimerNombre(newData.getPrimerNombre());
        if (newData.getSegundoNombre() != null) cliente.setSegundoNombre(newData.getSegundoNombre());
        if (newData.getPrimerApellido() != null) cliente.setPrimerApellido(newData.getPrimerApellido());
        if (newData.getSegundoApellido() != null) cliente.setSegundoApellido(newData.getSegundoApellido());
        if (newData.getFechaNacimiento() != null) cliente.setFechaNacimiento(newData.getFechaNacimiento());
        if (newData.getAnosComprando() != null) cliente.setAnosComprando(newData.getAnosComprando());
        if (newData.getContrasenia() != null) cliente.setContrasenia(AESUtil.hashinBCrypt(newData.getContrasenia()));
        if (newData.getTelefono() != null) cliente.setTelefono(newData.getTelefono());
        if (newData.getDireccion() != null) cliente.setDireccion(newData.getDireccion());

        if (newData.getpedidosId() != null) {
            List<Pedido> pedidos = newData.getpedidosId().stream()
                    .map(pid -> pedidoRepo.findById(pid).orElse(null))
                    .filter(Objects::nonNull)
                    .collect(Collectors.toList());
            cliente.setPedidos(pedidos);
